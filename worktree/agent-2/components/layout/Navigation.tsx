...
function NavLink({ href, icon, label }: NavLinkProps) {
  return (
    <Link
      href={href}
      className="flex items-center gap-1.5 px-3 py-2 text-gray-300 hover:text-neon-green hover:bg-cyber-gray-light rounded-md transition-all border border-transparent hover:border-neon-green/30 hover:shadow-neon-green text-sm"
    >
      {icon}
      <span>{label}</span>
    </Link>
